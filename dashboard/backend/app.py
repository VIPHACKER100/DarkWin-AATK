...
def create_app(reports_dir: str = None, logs_dir: str = None):
    ...
    def list_targets():
        ...
        for d in sorted(base.iterdir()):
            if not d.is_dir():
                continue
            # Skip directories created from malformed targets (e.g. "http:",
            # "https:") which cannot be served or deleted safely.
            if safe_target(d.name) != d.name:
                continue
            sessions = []
    ...
    def start_scan():
        ...
        if not target:
            return _err("target is required and must be a valid host/IP (a bare scheme or URL is not accepted)")
        ...
        _current_scan.update(scan_id=scan_id, target=target, mode=mode, status="running", started_at=datetime.now(timezone.utc).isoformat(), phase="initializing", progress=0)
        ...
        def _run():
            ...
            try:
                ...
                # Show a live progress bar in the server console when the
                # backend was started interactively (scan triggered over HTTP).
